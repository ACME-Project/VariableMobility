...
int main(int argc, char* argv[])
{
  ...
  if (scalar_type) {
	  ...
	  if (float_type) {
		 	...
		 	altfile.append("_mob.png");
	  	// Image |{φ}|
	  	MMSP::grid<2,MMSP::sparse<float> > grid(argv[1]);
		 	std::cout<<"Rescaling for mobility in [0, 1]."<<std::endl;
 			for (int n=0; n<nodes(grid); ++n) {
 				double phi = grid(n).getMagPhi();
 				float mob = mobility<2>(0.0, 1.0, phi); // going to rescale anyway
				buffer[n] = 255*mob;
		 	}
		} else if (double_type) {
	  	// Image |{φ}|
